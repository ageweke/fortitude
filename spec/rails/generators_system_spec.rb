describe "Rails generator support", :type => :rails do
  ...
  describe "base view generation" do
    it "should be able to generate a Views::Base file" do
      clean_views_base!
      rails_server.run_command_in_rails_root!("rails generate fortitude:base_view")
  ...
  describe "mailer generation" do
    it "should be able to generate a mailer that creates a Fortitude view and layout" do
      rails_server.run_command_in_rails_root!("rails generate mailer gen1 act_ion1 -e fortitude")

      view_file_path = File.join(rails_server.rails_root, 'app', 'views', 'gen1_mailer', 'act_ion1.html.rb')
      expect(File.exist?(view_file_path)).to be_truthy
      expect(File.read(view_file_path)).to match(/Views::Gen1Mailer::ActIon1\s*<\s*Views::Base/)
    end
